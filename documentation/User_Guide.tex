...
uptaken and secreted over its lifetime, conserving mass with the uptake/secretion terms in the 
...
& = & \Delta t \int_{\Omega_n } V_i \delta( \vec{x} - \vec{x}_i ) 
\Bigl( \vec{S}_i \circ 
\left( \grvec{\rho}^*_i - \grvec{\rho} \right) - \vec{U}_i \circ \grvec{\rho} \Bigr) dV 
\nonumber \\
& = & 
\Delta t V_i 
\Bigl( \vec{S}_i \circ 
\left( \grvec{\rho}^*_i - \grvec{\rho}(\vec{x}_n) \right) - \vec{U}_i \circ \grvec{\rho}(\vec{x}_n) \Bigr)   
\eeqa
...
\beq
\Delta \vec{n}_k = W_k 
\left( \frac{(\grvec{1}-\vec{c}_2) \circ \grvec{\rho}_k + \vec{c}_1 }{ \vec{c}_2 } \right), 
...
\item 
\smallcode{void differentiate(Cell* parentCell, Cell* daughterCell)} is called by the \smallcode{divide} function to determine whether a cell is allowed to differerentiate, and if so, what cell types the two daughter cells will take on. See \ref{sec:Differentiation} for more on cell differentiation.
...
\item 
\smallcode{void convert\_to\_cell\_definition\_preserving\_volume( Cell\_Definition\& cd )} functions the same as \smallcode{convert\_to\_cell\_definition}, except volume goes unchanged.
...
separately.  See \ref{sec:Differentiation} for more on cell differentiation.
...
\item 
\textbf{Differentiation} defines whether a cell can differentiate into other cell types, and if so, what possible differentiation outcomes can occur. See Section \ref{sec:Differentiation}. 
\item 
\textbf{Differentiation\_Ouctome} defines the cell types that result from a possible differentiation outcome. See Section \ref{sec:Differentiation_Outcome}. 
...
\subsection{Differentiation \DONE}
\label{sec:Differentiation}
\v|Differentiation| contains information about a Cell's ability (or lack there of) to convert to other cell types during cell division. Its main data elements include:
\begin{enumerate}
\item 
\smallcode{bool differentiation\_possible} this variable should be set to true if it is possible for the cell to differentiate during cell divison. Set to false by default.
\item 
\smallcode{std::vector<Differentiation\_Outcome> outcomes} when a cell with \smallcode{differentiation\_possible} set to true divides, one outcome will be chosen from this vector to determine the cell type of each of the two daughter cells that result. See \ref{sec:Differentiation_Outcome} for more information.
\item 
\smallcode{std::vector<double> probabilities} this vector sets the probability of each possible differentiation outcome. The size of the vector should be equal to the size of \smallcode{outcomes}, and the elements of this vector should sum to 1. 
\end{enumerate}
Here is the full class declaration:
\begin{verbatim}
...
\begin{enumerate}
\item 
\smallcode{Differentatiation()} is the default constructor. It sets \smallcode{differentiation\_possible} to false.
\end{enumerate}
\TOClink
\subsection{Differentiation\_Outcome \DONE}
\label{sec:Differentiation_Outcome}
\v|Differentiation_Ouctome| contains two \smallcode{Cell\_Definition} objects. When a cell that is allowed to differentiate divides, a \smallcode{Differentiation\_Outcome} is chosen to determine the cell types that two daughter cells will take on. See  \ref{sec:Differentiation} for more information on cell differentiation. Its main data elements include:
\begin{enumerate}
\item 
\smallcode{Cell\_Definition* first\_type} the cell type that one of the two daughter cells will take on when this outcome occurs.
\item 
\smallcode{Cell\_Definition* second\_type} the cell type that the other daughter cell will take on when this outcome occurs.
\end{enumerate}
Note that the ordering of these two variables is arbitrary and should not have any meaningful impact, as which daughter cell recieves which of the two types is random.
Here is the full class declaration:
\begin{verbatim}
...
\begin{enumerate}
\item 
\smallcode{Differentiation\_Outcome(Cell\_Definition*, Cell\_Definition*)} is the default constructor. It sets both class variables.
\end{enumerate}
\TOClink
