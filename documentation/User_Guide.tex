...
\hypersetup{
pdftitle={PhysiCell User Guide 1.2.0}, 
...
\date{Revision: July 30, 2017}%\today}
...
[1] A. Ghaffarizadeh, S.H. Friedman, S.M. Mumenthaler, and P. Macklin, PhysiCell: an Open Source Physics-Based Cell Simulator for 3-D Multicellular Systems, 
bioRxiv 088773, 2016. DOI: 
...
complex installation. In principle, any C++11 compliant compiler with OpenMP support should work. In 
practice, we use g++ as our ``gold standard.'' You'll also want to ensure that your build environment supports 
makefiles. Command-line zip/unzip is also helpful, but not required. 

Please note that OSX (and its associated developer tools) 
ships with ``g++'' instead of g++: it uses LLVM/Clang and an alias to \emph{pretend} to be g++. Unfortunately, 
the version of LLVM/Clang that Apple ships does not fully support OpenMP, and so compiling can 
...
Full tutorials on installing a 64-bit g++/OpenMP environment (on Windows via mingw-w64 and on OSX using Homebrew) 
can be found at: 

\href{http://www.mathcancer.org/blog/physicell-tutorials/}{http://www.MathCancer.org/blog/physicell-tutorials/}

Linux users should already have a 64-bit g++ environment installed by default. 
...
\href{https://www.docker.com/}{Docker} containers, and virtual appliances 
for use in \href{http://virtualbox.org}{VirtualBox} and other virtual machine software, 
allowing us to distribute a full PhysiCell 
development environment (including 64-bit g++, support for makefiles, zip/unzip, 
...
or have difficulty installing and configuring g++. 
...
development environment (including 64-bit g++, support for makefiles, zip/unzip, a text editor, and the most up-to-date 
version of PhysiCell). This should make it simpler to start learning and using PhysiCell, even in cases 
where developers are not free to install or modify their own build environments, 
or have difficulty installing and configuring g++. 
...
Working in PhysiCell requires some knowledge of C++, Makefiles, and command-line routines in Unix-like systems. 
...
\item 
\smallcode{config} While this directory is not currently used, PhysiCell will eventually parse XML files with 
configuration information in this directory. Any custom configuration files should be placed in this directory. 
\item 
...
\item 
\smallcode{template\_projects} This directory stores basic templates for 2D and 3D projects. A good start to a new 
project is to use the \v|make template2D| or \v|make template3D| makefile rules, which will copy the appropriate template 
to main.cpp. See Section \ref{sec:templates}.
\end{enumerate}
...
This will copy \v|template_projects/template2D.cpp| to \v|main.cpp| in the root directory. From here, modify main.cpp 
as you see fit, add new custom codes in the \v|custom_modules| directory, and edit the \v|Makefile| as needed to include 
these codes. Once you're ready, type 

\v|make|

to build your new project. By default, the executable will be called \v|project| (\v|project.exe| on windows). To 
...
\v|initialize_microenvironment()| without setting units, 
...
$0.1 1/\micron$ (for a 1 mm diffusion length scale 
...
As of Version 1.2.0, PhysiCell has automated cell secretions/uptake, so there is no need to explicitly call the BioFVM cell-based functions in the main program loop.

Note that the PhysiCell templates already include these critical setup commands. 
...
function is case sensitive. Here's 
...
\smallcode{void Microenvironment::update\_dirichlet\_node( int voxel\_index ,
 ...
 value of the Dirichlet node at \v|voxel\_index|, so that 
...
build in some hysteresis (bias towards the last directoin of the day).
