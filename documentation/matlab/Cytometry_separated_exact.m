...
function solution = Cytometry_separated_exact( parameters )
...
solution.G2 = zeros( 1 , length(solution.T)); 
solution.M = zeros( 1 , length(solution.T)); 
...
solution.G2I = zeros(1,length(solution.T)); 
solution.MI = zeros(1,length(solution.T)); 
...
TG2 = parameters.G2.duration; 
rG2A = parameters.G2.death_rate; 

TM = parameters.M.duration; 
rMA = parameters.M.death_rate; 
...
Operator = [ -(1/TG0G1 +rG0G1A) , 0 , 0, 2/TM , 0; ...
    1/TG0G1 , -(1/TS + rSA) ,0 , 0, 0; ...
    0 , 1/TS , -(1/TG2 + rG2A) , 0, 0 ; ... 
    0 , 0 , 1/TG2 , -(1/TM + rMA) , 0; ... 
    rG0G1A , rSA, rG2A , rMA, -1/TA ] ;
...
VecNow = [ parameters.G0G1.initial ; parameters.S.initial ; ... 
    parameters.G2.initial ; parameters.M.initial; parameters.A.initial ] ; 

solution.G0G1(1) = VecNow(1); 
solution.S(1) = VecNow(2); 
solution.G2(1) = VecNow(3); 
solution.M(1) = VecNow(4); 
solution.A(1) = VecNow(5); 

% solution.K(1) = solution.K1(1) + solution.K2(1); 
solution.Live(1) = sum( VecNow(1:4) ); 
solution.Total(1) = sum( VecNow(1:5) ); 
...
solution.G2I(1) = solution.G2(1) / solution.Total(1); 
solution.MI(1) = solution.M(1) / solution.Total(1); 
...
solution.long_time.G2I = V(3,n) / sum( V(:,n) ); 
solution.long_time.MI = V(4,n) / sum( V(:,n) ); 
solution.long_time.AI = V(5,n) / sum( V(:,n) ) ;
...
for i=2:length( solution.T )
    ...
    solution.G2(i) = VecExact(3); 
    solution.M(i) = VecExact(4); 
    solution.A(i) = VecExact(5); 
    solution.Live(i) = sum( VecExact(1:4) ); 
    solution.Total(i) = sum( VecExact(1:5) ); 
    ...
    solution.G2I(i) = solution.G2(i) / solution.Total(i); 
    solution.MI(i) = solution.M(i) / solution.Total(i); 
